...
lonCM=30 #derece CENTRAL MERIDIAN
lat=dms2deg("69 36 4.39390")#derece LATITUDE fi
lon=dms2deg("30 0 0") #derece LONGTITUDE lamda
...
x=(lon*cos(lat)+((lon**3*cos(lat)**3)/6)*(1-t**2+n**2)+
   ((lon**5*cos(lat)**5)/120)*(5-18*t**2+t**4+14*n**2-58*t**2*n**2
   +13*n**4+4*n**6-64*n**4*t**2-24*n**6*t**2)+
   ((lon**7*cos(lat)**7)/5040)*(61-479*t**2+179*t**4-t**6))*N

   
y=((Slat/N)+(lon**2*sin(lat)*cos(lat))/2+((lon**4*sin(lat)*cos(lat)**3)*(5-
    t**2+9*n**2+4*n**4))/24+
